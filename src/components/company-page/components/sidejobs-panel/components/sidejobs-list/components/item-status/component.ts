...
import { ACTIVITY_UI_STATUS_VALUES, ACTIVITY_UI_STATUSES, ActivityUIStatus, BaseComponent } from '@shared/index';
...
export class SidejobsListItemStatus extends BaseComponent {
  ...
  handlePartialUpdate = () => {
    ...
    if (this._desktopIconRef.value) {
      this._desktopIconRef.value.name = ACTIVITY_UI_STATUS_VALUES[status].icon;
  ...
  private getStatus(validationResult: SidejobValidationResult): ActivityUIStatus {
    if (this._activity!.active) {
      return ActivityUIStatus.active;
    }

    if (validationResult === SidejobValidationResult.valid) {
      return ActivityUIStatus.paused;
    }

    return ActivityUIStatus.invalid;
  }

  private getText(validationResult: SidejobValidationResult): string {
    if (this._activity?.active) {
  ...
  private applyStatusClass(element: HTMLElement, status: ActivityUIStatus) {
    ACTIVITY_UI_STATUSES.forEach((activityStatus) =>
      element.classList.remove(ACTIVITY_UI_STATUS_VALUES[activityStatus].class),
    );

    element.classList.add(ACTIVITY_UI_STATUS_VALUES[status].class);
  }
}
